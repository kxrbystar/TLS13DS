                                       ...
                                       size_t *binders_len)
{
    ...
    *p++ = MBEDTLS_SSL_COMPRESS_NULL;
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
    if (propose_tls12) {
        ret = mbedtls_ssl_tls12_write_client_hello_exts_pc_order(
            ssl, p, end, tls12_uses_ec, 0, &output_len);
        if (ret != 0) {
            return ret;
        }
        p += output_len;
    ...
    if (propose_tls12) {
        ret = mbedtls_ssl_tls12_write_client_hello_exts_pc_order(
            ssl, p, end, tls12_uses_ec, 1, &output_len);
        if (ret != 0) {
            return ret;
        }
        p += output_len;
    ...
    if (propose_tls12) {
        ret = mbedtls_ssl_tls12_write_client_hello_exts_pc_order(
            ssl, p, end, tls12_uses_ec, 2, &output_len);
        if (ret != 0) {
            return ret;
        }
        p += output_len;

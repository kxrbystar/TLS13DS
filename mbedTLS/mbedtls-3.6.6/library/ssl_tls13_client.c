...
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_PSK_ENABLED)
    /* Put psk_key_exchange_modes before key_share to better match common PC
     * TLS ClientHello ordering. pre_shared_key, when present, remains handled
     * separately as the final extension by the generic ClientHello writer. */
